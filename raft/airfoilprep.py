...
class Polar(object):
    ...
    def __init__(self, Re, alpha, cl, cd, cm):
        ...
        self.Re = Re
        if len(alpha) < 3:
            self.alpha = np.linspace(alpha[0], alpha[1], 3, endpoint=True)
            self.cl = np.ones_like(self.alpha) * cl[0]
            self.cd = np.ones_like(self.alpha) * cd[0]
            self.cm = np.ones_like(self.alpha) * cm[0]
        else:
